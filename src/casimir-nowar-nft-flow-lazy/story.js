...
async function run() {
  ...
  const bobDaoCoreAssetBalance1 = await rpc.getFungibleTokenBalanceByOwnerAsync(bobDaoId, DEIP_APPCHAIN_CORE_ASSET.id);
  logJsonResult(`Alice Dao CoreAsset balance after CreateProjectCmd`, aliceDaoCoreAssetBalance1);
  logJsonResult(`Bob Dao CoreAsset balance after CreateProjectCmd`, bobDaoCoreAssetBalance1);

  const aliceBobTransferTx = await chainTxBuilder.begin()
    .then((txBuilder) => {
  ...
  const aliceBobTransferByAliceTx = await aliceBobTransferTx.signAsync(getDaoCreatorPrivKey(alice), api); // 1st approval from Alice DAO (final)
  await sendTxAndWaitAsync(aliceBobTransferByAliceTx);


  const bobAliceTransferTx = await chainTxBuilder.begin()
  .then((txBuilder) => {
    const transferFt1 = new TransferFungibleTokenCmd({
      from: bobDaoId,
      to: aliceDaoId,
...
const bobAliceTransferByBobTx = await bobAliceTransferTx.signAsync(getDaoCreatorPrivKey(bob), api); // 1st approval from Alice DAO (final)
await sendTxAndWaitAsync(bobAliceTransferByBobTx);


const aliceDaoCoreAssetBalance2 = await rpc.getFungibleTokenBalanceByOwnerAsync(aliceDaoId, DEIP_APPCHAIN_CORE_ASSET.id);
const bobDaoCoreAssetBalance2 = await rpc.getFungibleTokenBalanceByOwnerAsync(bobDaoId, DEIP_APPCHAIN_CORE_ASSET.id);
logJsonResult(`Alice Dao CoreAsset balance 2`, aliceDaoCoreAssetBalance2);
logJsonResult(`Bob Dao CoreAsset balance 2`, bobDaoCoreAssetBalance2);
...
//   /**
//    * Create Project Content on behalf of Creator DAO actor
//    */
//    logInfo(`Creating Creator Project-1 Project-Content-1 ...`);
...
//        return txBuilder.end();
//      });
//    const createProject1ProjectContent1ByCreatorDaoTx = await createProject1Content1Tx.signAsync(creator.getPrivKey(), api); // 1st approval from Creator DAO (final)
//    await sendTxAndWaitAsync(createProject1ProjectContent1ByCreatorDaoTx);
//    logJsonResult(`Creator DAO Project-1 created`, project1);
...
//   const nft1Id = 2342348223//genRipemd160Hash(randomAsHex(20));
